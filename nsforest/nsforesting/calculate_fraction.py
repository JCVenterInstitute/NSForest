...
def markers_onTarget(adata, markers_dict, cluster_header, medians_header, use_mean = False, output_folder = "", outputfilename = ""):
    ...
    for cl in markers_dict.keys():
        ...
        df_ontarget_supp.to_csv(output_folder + outputfilename + "_markers_onTarget_supp.csv", index=False)
    
    df_ontarget = df_ontarget_supp[['clusterName', 'onTarget']].drop_duplicates().reset_index(drop=True)
    df_ontarget.to_csv(output_folder + outputfilename + "_markers_onTarget.csv", index=False)
    return df_ontarget

# Calculate ratio of diagonal/total expression
def on_target_fraction_angela(adata, markers_dict, cluster_header, medians_header, output_folder, outputfilename): 
    """\
